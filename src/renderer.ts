...
export function RequestRender(): void {
    modeToggleBtn.textContent = view.plugin.settings.mode == 'edit' ? 'Edit Mode' : 'View Mode';
}
...
export function Render(): void {
    ...
    //         view._cachedThemeColors = {
    //             text: docStyle.getPropertyValue('--text-normal')?.trim() || '#000',
    //             bg: docStyle.getPropertyValue('--background-primary')?.trim() || '#fff',
    //             border: docStyle.getPropertyValue('--background-modifier-border')?.trim() || '#ccc',
    //             accent: docStyle.getPropertyValue('--interactive-accent')?.trim() || '#0066cc'
    //         };
    //     } catch (e) {
    //         view._cachedThemeColors = { text: '#000', bg: '#fff', border: '#ccc', accent: '#0066cc' };
    //     }
    // }
