...
const VIEW_TYPE_SKILLTREE = 'skill-tree-view';
...
function drawArrow(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number, scale = 1) {
  ctx.beginPath();
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
  ctx.stroke();
...
function drawBezierArrow(ctx: CanvasRenderingContext2D, p0x: number, p0y: number, p1x: number, p1y: number, p2x: number, p2y: number, p3x: number, p3y: number, scale = 1) {
  ctx.beginPath();
  ctx.moveTo(p0x, p0y);
...
function distanceSqToBezier(x: number, y: number, p0x: number, p0y: number, p1x: number, p1y: number, p2x: number, p2y: number, p3x: number, p3y: number, samples = 24) {
...
function computeBezierControls(ax: number, ay: number, bx: number, by: number, fromSide: any, toSide: any, rFrom: number, rTo: number) {
  const dx = bx - ax;
  const dy = by - ay;
...
function parseCSSColor(s: string) {
  s = s.trim();
  if (!s) return null;
  if (s.startsWith('#')) {
    const hex = s.slice(1);
...
function defaultNodes(): SkillNode[] {
  return [
    { id: Date.now(), x: 200, y: 150, label: 'Core Skill', state: 'in-progress', exp: 0 },
  ];
}
...
class SkillTreeView extends ItemView {
  ...
  resize(): void {
    if (!this.canvas || !this.ctx || !this.canvasWrap) return;
  ...
  computeNodeRadius(n: SkillNode): number {
    if (!this.ctx) return this.settings.nodeRadius;
    try {
      this.ctx.save();
      ...
      const titleWidth = this.ctx.measureText(labelText).width || 0;
      ...
      if (n.fileLink) {
        ...
        fileNameWidth = this.ctx.measureText(fileName).width || 0;
        this.ctx.font = '14px sans-serif'; // Reset
      }
      ...
      try { this.ctx && this.ctx.restore(); } catch (e2) {}
...
export default class SkillTreePlugin extends Plugin {
  ...
  async activateView() {
    ...
    await leaf.setViewState({ type: VIEW_TYPE_SKILLTREE });
